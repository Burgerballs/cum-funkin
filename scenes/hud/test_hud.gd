...
@onready var rating: Sprite2D = $CanvasLayer/rating
...
func on_note_rate(player:Player,_rating:NoteRating):
	...
	rating.position = get_viewport().get_camera_2d().get_target_position()
	rating.modulate = Color.WHITE
	if is_instance_valid(rat_tween):
		rat_tween.kill()
	rat_tween = create_tween().set_parallel()
	rat_tween.tween_property(rating,"position:y",rating.position.y - 15,Conductor.beat_crochet).set_trans(Tween.TRANS_SPRING).set_ease(Tween.EASE_OUT)
	rat_tween.tween_property(rating,"modulate:a",0.0,Conductor.beat_crochet).set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_OUT).set_delay(Conductor.beat_crochet)
	pass
